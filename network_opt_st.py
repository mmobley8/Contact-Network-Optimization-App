...
# file_io = io.TextIOWrapper(uploaded_file)
...
def opt():
    series_list = [pd.Series(connections["First Name"]), pd.Series(connections["Last Name"]), pd.Series(connections["Company"]), pd.Series(connections["Position"])]
    connections_new = pd.concat(series_list, axis = 1)
    st.write(connections_new)

    spark_connections = spark.createDataFrame(connections_new)


    spark_connections.select("*",
